import { PacketType, PacketMap } from 'realmlib';

/**
 * Numeric packet id -> realmlib PacketType for the current build (6.7.x).
 * Sourced from a current working client (pyrelay's Constants/PacketIds.py)
 * and translated to realmlib's PacketType enum.
 */
...
/**
 * Bidirectional map (id <-> type) consumed by realmlib's PacketIO.
 */
export const PACKET_MAP: PacketMap = {};
for (const [id, type] of Object.entries(ID_TO_TYPE)) {
  PACKET_MAP[Number(id)] = type;
  PACKET_MAP[type] = Number(id);
}
